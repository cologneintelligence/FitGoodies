...
import fitgoodies.parsers.ParserHelper;
...
public final class FixtureTools {
	...
	public static TypeAdapter processCell(final Parse cell,
		...
		try {
			cell.body = CrossReferenceHelper.instance().parseBody(cell.body,
					actualStringValue);
	...
	public static Object parse(final String text, final Class<?> type,
		...
		return ParserHelper.instance().parse(text, type, parameter);
	}
	...
	public static TypeAdapter rebindTypeAdapter(final TypeAdapter ta,
			final String parameter) {
		return TypeAdapterHelper.instance().getAdapter(ta, parameter);
	}
		...
		for (String argument : args) {
			...
			if (pair.length == 2) {
				if (pair[0].trim().equalsIgnoreCase(argName)) {
					try {
						return CrossReferenceHelper.instance().parseBody(
								pair[1].trim(), "");
					} catch (CrossReferenceProcessorShortcutException e) {
						return "";
					}
